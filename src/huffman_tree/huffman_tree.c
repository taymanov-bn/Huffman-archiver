...
int* frequency_counter(const char* filename) { // функция по подсчету частот
  ...
  int* frequency_array = calloc(256, sizeof(int)); // выделяем память для массива с частотами (calloc чтобы обнулить все значения внутри)
  ...
  return frequency_array; // возвращаем массив с частотами


}

Heap* heap_build_freq(int* frequency) { // функция для построения узлов и добавления их в массив кучи 
  if (!frequency) {
  ...
  for (int i = 0; i < 256; i++) { // цикл для создания узлов и добавления их в массив кучи 
    if (frequency[i] > 0) { // ну и создаем узлы только для частот > 0
      Node* node = create_node(i, frequency[i], NULL, NULL); // создание самих узлов 
      if (!node) {
...
char* dfs_codes (Node* root, char* buffer, char** code, int* index) { // функция обхода в глубину дерева для построения таблицы кодов 
  if (!root) return NULL;
  ...
  if (root-> left == NULL && root->right == NULL) { // проверка если детей нет, в конец буфера ставим \0 и копируем весь буффер в таблицу кодов
    ...
    buffer[*index] = '\0'; // добавляем нуль-терминатор в конец для понимания, что это конец массива
    memcpy(code[root->symbol], buffer, *index + 1); // копируем в code из буффера
    return code[root->symbol];

  }
  buffer[*index] = '0'; // добавляем 0 если идет налево 
  (*index)++; // увеличиваем индекс
  dfs_codes(root->left, buffer, code, index); // через рекурсию идем к левому ребенку 
  (*index)--; // откатываем индекс после возврата из рекурсии  
  buffer[*index] = '1'; // аналогично но к правому ребенку 
  (*index)++;
  dfs_codes(root->right, buffer, code, index);
  (*index)--; // аналогично, откатываем индекс после возврата из рекурсии
  return NULL;
