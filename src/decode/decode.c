...
int decode (char* file_input, char* file_output) {
  ...
  if (!fread(frequency_array, sizeof(int), FREQUENCY_SIZE, inp_file)) { // проверка и читаем первые 256 * sizeof(int) байт в массив частот
  ...
  Heap* heap = heap_build_freq(frequency_array); // создаем кучу из массива частот 
  if (!heap) {
  ...
  while (fread(&byte, sizeof(char), 1, inp_file) == 1) { // читаем байт и декодируем 
    if (frequency_counter == 0) break; // если обшее колво частот = 0, выходим с цикла 
      ...
      if (buffer_byte == 0) { // если бит равен 0
        if (current_node->left != NULL) { // переходим к левому ребенку 
          ...
          if (current_node->left == NULL && current_node->right == NULL) { // если у текущего узла нет детей, то записываем символ(байт) в выходной файл
            ...
            frequency_counter--; // уменьшаем колво частот символов 
            if (frequency_counter == 0) break; // вот и проверка для которой нужна эта перемення, если она равна 0, ты прерываем цикл 
