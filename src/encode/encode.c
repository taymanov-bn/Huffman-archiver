...
int encode(char* file_input, char* file_output) { // функция для кодирования файла 
  ...
  int* frequency = frequency_counter(file_input); // создаем массив частот из входного файла 
  if (!frequency) {
  ...
  Heap* heap = heap_build_freq(frequency); // создаем кучу с узлами из массива частот 
  if (!heap) {
  ...
  if (!fwrite(frequency, sizeof(int), FREQUENCY_SIZE, out_file)) { // записываем массив частот в выходной файл, чтобы при декодировании могли заново построить дерево Хаффмана 
